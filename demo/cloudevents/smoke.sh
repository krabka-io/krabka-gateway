...
gateway_url="${KRABKA_GATEWAY_URL:-http://127.0.0.1:9500}"
topic="${KRABKA_CE_TOPIC:-cloudevents-demo}"
capture_port="${KRABKA_CE_CAPTURE_PORT:-18080}"
capture_dir="$(mktemp -d "${TMPDIR:-/tmp}/krabka-ce-smoke.XXXXXX")"
...
post_binary_cloudevent() {
  curl -fsS -X POST "${gateway_url}/v1/produce/${topic}" \
    ...
    -H 'ce-type: com.example.krabka.binary' \
...
post_structured_cloudevent() {
  curl -fsS -X POST "${gateway_url}/v1/webhooks/events" \
    ...
    --data '{"specversion":"1.0","id":"demo-structured-1","source":"/demo/cloudevents/smoke.sh","type":"com.example.krabka.structured","datacontenttype":"application/json","data":{"mode":"structured","ok":true}}' >/dev/null
}
...
while time.monotonic() < deadline:
    ...
    saw_binary = any(
        ...
        and request["headers"].get("ce-type") == "com.example.krabka.binary"
