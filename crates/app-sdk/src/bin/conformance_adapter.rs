...
#[derive(Default)]
struct Adapter {
    client: Option<KrabkaClient>,
    stream: Option<krabka_app_sdk::MessageStream>,
    ...
    async fn handle(&mut self, command: Command) -> Response {
        match command {
            ...
            Command::Configure { endpoint, bearer } => {
                let mut builder = KrabkaClient::builder().endpoint(endpoint);
            ...
            Command::Publish {
                ...
                let Some(client) = &self.client else {
                    return error_response(KrabkaError::InvalidArgument(
                        "configure before publish".into(),
                    ));
                };
                let value = match STANDARD.decode(value_b64) {
                    Ok(value) => Bytes::from(value),
                    Err(e) => return error_response(KrabkaError::InvalidArgument(e.to_string())),
                };
            ...
            Command::NextMessage { timeout_ms } => {
                ...
                .await
                {
                    ...
                    Ok(None) => {
                        error_response(KrabkaError::NotFound("no message available".into()))
                    }
                    Err(_) => error_response(KrabkaError::NotFound("no message available".into())),
                }
            }
            ...
            Command::QueueAck { message_id } => {
                let _ = message_id;
                error_response(KrabkaError::Unimplemented {
                    module: "queues",
...
fn queue_ack_entry(
    message_id: &str,
    ack_type: krabka_sdk_conformance::protocol::QueueAckType,
) -> Result<QueueAckEntry, KrabkaError> {
    ...
    Ok(QueueAckEntry {
        ...
        ack_type: match ack_type {
            krabka_sdk_conformance::protocol::QueueAckType::Accept => QueueAckType::Accept,
            krabka_sdk_conformance::protocol::QueueAckType::Release => QueueAckType::Release,
            krabka_sdk_conformance::protocol::QueueAckType::Reject => QueueAckType::Reject,
        },
    })
}

fn queue_renew_entry(message_id: &str) -> Result<QueueRenewEntry, KrabkaError> {
...
fn queue_results(results: Vec<krabka_app_sdk::QueueAckResult>) -> Vec<serde_json::Value> {
    results
        .into_iter()
        .map(|result| {
            json!({
...
fn adapter_error_value(error: KrabkaError) -> serde_json::Value {
    let error = match error {
        KrabkaError::InvalidArgument(message)
            if message == "record is not acquired by this session" =>
        {
            KrabkaError::InvalidArgument("queue message is not acquired".into())
        }
        error => error,
    };
...
fn error_response(error: KrabkaError) -> Response {
    let wire = match error {
        KrabkaError::Transport(message) => {
            AdapterError::with_message(ErrorKind::Transport, message)
        }
        KrabkaError::Unauthenticated(message) => {
            AdapterError::with_message(ErrorKind::Unauthenticated, message)
        }
        KrabkaError::InvalidArgument(message) => {
            AdapterError::with_message(ErrorKind::InvalidArgument, message)
        }
        KrabkaError::NotFound(message) => AdapterError::with_message(ErrorKind::NotFound, message),
        KrabkaError::ServerError(message) => {
            AdapterError::with_message(ErrorKind::ServerError, message)
        }
        KrabkaError::Unimplemented { module, gated_on } => {
