...
use crate::{
    client::{
        KrabkaClient, MockMessage, MockQueueDelivery, MockQueueDisposition, MockQueueMessageState,
        MockQueueSession,
    },
    error::KrabkaError,
    pb,
...
impl QueuesClient {
    pub(crate) fn new(client: KrabkaClient) -> Self {
        Self { client }
    }
    ...
    ) -> Result<QueueAcquireResult, KrabkaError> {
    ...
    pub async fn acquire_with_session(
        ...
        if group.is_empty() {
            return Err(KrabkaError::InvalidArgument(
        ...
        if options.lock_duration_ms != SUPPORTED_QUEUE_LOCK_DURATION_MS {
            return Err(KrabkaError::InvalidArgument(format!(
        ...
        if self.client.is_unreachable() {
            return Err(KrabkaError::Transport("endpoint unreachable".into()));
        }
    ...
    ) -> Result<Vec<QueueAckResult>, KrabkaError> {
        let session_id = session_id.into();
        if self.client.is_mock() {
...
impl From<pb::QueueAckResult> for QueueAckResult {
    fn from(value: pb::QueueAckResult) -> Self {
        ...
        Self {
            ...
            error: value.error.map(error_info_to_krabka),
        }
    }
}

fn error_info_to_krabka(error: pb::ErrorInfo) -> KrabkaError {
    let message = error.message;
    if error.retriable {
        return KrabkaError::Transport(message);
    }
    match error.code {
        3 | 9 => KrabkaError::InvalidArgument(message),
        16 => KrabkaError::Unauthenticated(message),
        5 => KrabkaError::NotFound(message),
        _ => KrabkaError::ServerError(message),
    }
}
...
fn queue_result_error(entry: QueueRenewEntry, message: impl Into<String>) -> QueueAckResult {
    QueueAckResult {
        entry,
        error: Some(KrabkaError::InvalidArgument(message.into())),
    }
}
...
impl DatabaseClient {
    ...
    /// # Errors
    ///
    /// Always returns [`KrabkaError::Unimplemented`] while the database module
    /// remains gated.
    pub fn connect(&self, _name: &str) -> Result<(), KrabkaError> {
        Err(KrabkaError::Unimplemented {
            module: "database",
...
impl AuthClient {
    ...
    /// # Errors
    ///
    /// Always returns [`KrabkaError::Unauthenticated`] because sign-in is not
    /// part of contract v1.
    pub fn sign_in(&self, _username: &str, _password: &str) -> Result<(), KrabkaError> {
        Err(KrabkaError::Unauthenticated(
...
impl BlobClient {
    ...
    /// # Errors
    ///
    /// Always returns [`KrabkaError::Unimplemented`] while the blob module
    /// remains gated.
    pub fn put(&self, _key: &str, _value: Bytes) -> Result<(), KrabkaError> {
        Err(KrabkaError::Unimplemented {
            module: "blob",
    ...
    /// Always returns [`KrabkaError::Unimplemented`] while the blob module
    /// remains gated.
    pub fn get(&self, _key: &str) -> Result<Bytes, KrabkaError> {
        Err(KrabkaError::Unimplemented {
            module: "blob",
            gated_on: "chapter-b-blob-api",
        })
...
mod tests {
    use super::*;
    use crate::KrabkaClient;

    #[tokio::test]
    async fn mock_queues_acquire_published_records() {
        let client = KrabkaClient::new("mock://gateway").unwrap();
        client
            .messaging()
            .publish(
                "work",
    ...
    async fn mock_queue_sessions_own_delivered_coordinates() {
        ...
        assert2::assert!(
            missing
                == Err(KrabkaError::InvalidArgument(
                    "queue session expired; re-acquire".into()
                ))
        );
        let changed = client
    ...
    fn retriable_queue_error_overrides_gateway_code_and_preserves_message() {
        assert2::assert!(
            error_info_to_krabka(pb::ErrorInfo {
                code: 9,
                message: "coordinator retry".into(),
                retriable: true,
            }) == KrabkaError::Transport("coordinator retry".into())
        );
        assert2::assert!(
            error_info_to_krabka(pb::ErrorInfo {
                code: 9,
                ...
                retriable: false,
            }) == KrabkaError::InvalidArgument("record is not acquired by this session".into())
        );
    }
