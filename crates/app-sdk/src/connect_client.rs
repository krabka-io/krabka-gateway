...
mod tests {
    ...
    async fn live_http_and_end_stream_errors_reach_the_sdk_taxonomy() {
        ...
        assert2::assert!(let KrabkaError::InvalidArgument(message) = KrabkaError::from(error));
        assert_eq!(message, "bad request");
        ...
        assert2::assert!(let KrabkaError::NotFound(message) = KrabkaError::from(error));
