...
use crate::{
    client::{KrabkaClient, MockMessage},
    error::KrabkaError,
    pb,
};
...
impl MessagingClient {
    ...
    pub async fn publish(
    ...
    ) -> Result<RecordResult, KrabkaError> {
        ...
        if topic.is_empty() {
            return Err(KrabkaError::InvalidArgument("topic is required".into()));
        }
        ...
        let response: pb::SendResponse = self
            ...
            .inner
            .connect
            .unary("/krabka.gateway.v1.Gateway/Send", &request)
            .await?;
        response
            .results
            .into_iter()
            ...
            .transpose()?
            .ok_or_else(|| KrabkaError::ServerError("gateway returned no record result".into()))
    }
    ...
    pub fn subscribe(
    ...
    ) -> Result<MessageStream, KrabkaError> {
        ...
        let messages = self
            ...
            .mock
            .lock()
            .map_err(|_| KrabkaError::ServerError("mock state lock poisoned".into()))?
            .messages
            .clone();
    ...
    fn publish_mock(
        ...
        if topic == "__missing_topic" {
            return Err(KrabkaError::NotFound("topic not found".into()));
        }
        let mut state = self
            .client
            .inner
    ...
    fn subscribe_live(
        ...
        if group.is_empty() {
            return Err(KrabkaError::InvalidArgument("group is required".into()));
        }
        ...
        tokio::spawn(async move {
            let mut inbound = match connect
                .streaming::<_, pb::Inbound>("/krabka.gateway.v1.Gateway/Subscribe", &request)
                .await
            {
                Ok(inbound) => inbound,
                Err(error) => {
                    let _ = tx.send(Err(KrabkaError::from(error))).await;
                    return;
                }
            };
            while let Some(message) = inbound.recv().await {
                let mapped = message.map(inbound_from_pb).map_err(KrabkaError::from);
...
impl Filter {
    fn to_gateway_filter(&self) -> Result<String, KrabkaError> {
...
pub struct MessageStream {
    rx: mpsc::Receiver<Result<Inbound, KrabkaError>>,
}

impl Stream for MessageStream {
    type Item = Result<Inbound, KrabkaError>;
...
fn gateway_filter_literal(value: &Value) -> Result<String, KrabkaError> {
    match value {
        Value::String(value) => Ok(format!("'{}'", value.replace('\'', "''"))),
        Value::Bool(value) => Ok(value.to_string()),
        Value::Number(value) => Ok(value.to_string()),
        Value::Null => Ok("null".into()),
        Value::Array(_) | Value::Object(_) => Err(KrabkaError::InvalidArgument(
...
fn record_result_from_pb(result: pb::RecordResult) -> Result<RecordResult, KrabkaError> {
    if let Some(error) = result.error {
        return Err(KrabkaError::from_connect_code(
            match error.code {
                3 => "invalid_argument",
                5 => "not_found",
                7 => "unauthenticated",
...
mod tests {
    ...
    async fn publish_event_rejects_empty_id() {
        ...
        assert_eq!(
            error,
            KrabkaError::InvalidArgument("CloudEvent id is required".into())
        );
    }
