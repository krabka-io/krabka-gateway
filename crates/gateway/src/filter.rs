...
const RESERVED_ROW_MARKER_FIELD: &str = "__krabka_row_marker";
...
pub(crate) enum FilterCompileError {
    ...
    #[error(
        "DataFusion subscription filter evaluation requires the gateway crate arrow feature: {0}"
    )]
    #[allow(
        dead_code,
...
fn arrow_field_needs_enum_symbol_projection(field: &arrow::datatypes::Field) -> bool {
    ...
    let is_enum_field = field
        ...
        || field.metadata().contains_key("krabka.enum.symbols");
    if !is_enum_field {
        return false;
    }
...
fn parse_arrow_enum_symbols(
    ...
    let Some(symbols) = field.metadata().get("krabka.enum.symbols") else {
        return Err(FilterCompileError::DataFusion(format!(
            "arrow enum field {} stores numeric values but has no krabka.enum.symbols descriptor metadata",
            field.name()
        )));
    };
    let parsed: serde_json::Value = serde_json::from_str(symbols).map_err(|_| {
        FilterCompileError::DataFusion(format!(
            "arrow enum field {} has invalid krabka.enum.symbols metadata",
            field.name()
        ))
    })?;
...
impl FieldPath {
    fn parse(input: &str) -> Result<Self, FilterCompileError> {
        ...
        if segments.len() != 1 {
            return Err(FilterCompileError::DataFusionUnavailable(
                "nested field predicates require the gateway crate arrow feature".to_string(),
            ));
        }
...
mod tests {
    ...
    fn reserved_row_marker_name_is_rejected_but_string_literal_is_allowed() {
        let marker_error = CompiledFilter::compile("__krabka_row_marker = true")
        ...
        assert!(filter_matches(
            "status = '__krabka_row_marker'",
            br#"{"status":"__krabka_row_marker"}"#,
        ));
    }
    ...
    fn arrow_bridge_json_shape_and_typed_rows_have_filter_parity() {
        use krabka_client_streams::columnar::serde::arrow::arrow_batch_to_filter_json_rows;
    ...
    fn datafusion_arrow_filter_quotes_nested_and_repeated_row_bridge_field_paths() {
        use krabka_client_streams::columnar::serde::arrow::json_rows_to_arrow_filter_batch;
    ...
    fn datafusion_arrow_filter_rejects_numeric_enum_dictionaries_without_symbols() {
        ...
        assert!(
            matches!(error, FilterCompileError::DataFusion(message) if message.contains("no krabka.enum.symbols"))
        );
    }
    ...
    fn datafusion_arrow_filter_rejects_invalid_enum_symbol_metadata() {
        ...
        enum_field.set_metadata(HashMap::from([
            ("krabka.enum".to_string(), "true".to_string()),
            ("krabka.enum.symbols".to_string(), "not-json".to_string()),
        ]));
        let batch = RecordBatch::try_new(
            Arc::new(Schema::new(vec![enum_field])),
            vec![Arc::new(
        ...
        assert!(
            matches!(error, FilterCompileError::DataFusion(message) if message.contains("invalid krabka.enum.symbols"))
        );
    }
