//! `krabka-gateway` binary entry point.
//!
...
#[command(
    name = "krabka-gateway",
    version,
    about = "gRPC / Connect-RPC + HTTP gateway into Krabka topics"
)]
struct Args {
    ...
    #[arg(
        long,
        env = "KRABKA_GATEWAY_CLIENT_DISPATCH_QUEUE_CAPACITY",
    ...
    #[arg(
        long,
        env = "KRABKA_GATEWAY_CLIENT_FRAME_MAX",
    ...
    #[arg(
        long,
        env = "KRABKA_GATEWAY_LISTEN_ADDR",
    ...
    #[arg(
        long,
        env = "KRABKA_GATEWAY_CLIENT_ID",
        default_value = "krabka-gateway"
    )]
    client_id: String,
    ...
    #[arg(
        long,
        env = "KRABKA_GATEWAY_DEDUP_TOPIC",
        default_value = "__krabka_grpc_dedup"
    )]
    dedup_topic: String,

    /// Dedup topic partition count.
    #[arg(long, env = "KRABKA_GATEWAY_DEDUP_PARTITIONS", default_value = "8")]
    dedup_partitions: PartitionCount,

    /// Dedup window, e.g. `1h`.
    #[arg(
        long,
        env = "KRABKA_GATEWAY_DEDUP_WINDOW",
        default_value = "1h",
    ...
    #[arg(long, env = "KRABKA_GATEWAY_DEDUP_OWNERSHIP_GROUP")]
    dedup_ownership_group: Option<String>,

    /// Transactional id prefix for the dedup path.
    #[arg(
        long,
        env = "KRABKA_GATEWAY_DEDUP_TXN_PREFIX",
        default_value = "krabka-gw-dedup"
    )]
    ...
    #[arg(long, env = "KRABKA_GATEWAY_AUTHZ", default_value = "off")]
    authz: String,
    /// Comma-separated super-user principal names that bypass ACL checks.
    #[arg(long, env = "KRABKA_GATEWAY_AUTHZ_SUPER_USERS", default_value = "")]
    authz_super_users: String,
    /// ACL-cache refresh interval, e.g. `30s`.
    #[arg(
        long,
        env = "KRABKA_GATEWAY_ACL_REFRESH_INTERVAL",
        default_value = "30s",
        value_parser = parse::positive_time
    )]
    ...
    #[arg(long, env = "KRABKA_GATEWAY_BEARER", default_value = "off")]
    bearer: String,
    /// JWT claim name to use as the principal name.
    #[arg(
        long,
        env = "KRABKA_GATEWAY_BEARER_PRINCIPAL_CLAIM",
        default_value = "sub"
    )]
    ...
    #[arg(
        long,
        env = "KRABKA_GATEWAY_BEARER_ALLOWABLE_CLOCK_SKEW",
        default_value = "30s",
    ...
    // ── Runtime policy ──────────────────────────────────────────────────────
    #[arg(long, env = "KRABKA_GATEWAY_INTERNAL_TOPIC_REPLICATION_FACTOR")]
    internal_topic_replication_factor: Option<PositiveI16>,
    #[arg(long, env = "KRABKA_GATEWAY_INTERNAL_TOPIC_ALLOW_REPLICATION_FALLBACK")]
    internal_topic_allow_replication_fallback: Option<bool>,
    /// Internal-topic creation timeout, e.g. `10s`.
    #[arg(
        long,
        env = "KRABKA_GATEWAY_INTERNAL_TOPIC_CREATE_TIMEOUT",
        value_parser = parse::positive_time
    )]
    ...
    #[arg(
        long,
        env = "KRABKA_GATEWAY_INTERNAL_TOPIC_SEGMENT",
        value_parser = parse::positive_time
    )]
    ...
    #[arg(
        long,
        env = "KRABKA_GATEWAY_INTERNAL_TOPIC_MIN_CLEANABLE_DIRTY_RATIO",
    ...
    #[arg(
        long,
        env = "KRABKA_GATEWAY_CONSUMER_POLL_TIMEOUT",
        value_parser = parse::positive_time
    )]
    consumer_poll_timeout: Option<Time>,
    #[arg(long, env = "KRABKA_GATEWAY_OWNERSHIP_WARMUP_EMPTY_POLLS")]
    ownership_warmup_empty_polls: Option<PositiveU32>,
    /// Readiness-watcher poll interval, e.g. `250ms`.
    #[arg(
        long,
        env = "KRABKA_GATEWAY_READINESS_POLL_INTERVAL",
        value_parser = parse::positive_time
    )]
    ...
    #[arg(
        long,
        env = "KRABKA_GATEWAY_PRODUCE_MAX_BODY",
    ...
    #[arg(
        long,
        env = "KRABKA_GATEWAY_FORWARD_MAX_BODY",
        value_parser = parse::positive_byte_size
    )]
    ...
    #[arg(long, env = "KRABKA_GATEWAY_BROKER_TLS_CERT")]
    ...
    #[arg(long, env = "KRABKA_GATEWAY_BROKER_TLS_KEY")]
    ...
    /// `RawCodec`.
    #[arg(long, env = "KRABKA_GATEWAY_SCHEMA_REGISTRY_URL")]
...
async fn main() -> anyhow::Result<()> {
    ...
    let otlp = krabka_telemetry::OtlpConfig::from_env(
        |k| std::env::var(k).ok(),
        &args.client_id,
    ...
    let tls = match (args.tls_cert.clone(), args.tls_key.clone()) {
        (Some(cert_chain_path), Some(private_key_path)) => {
            let client_auth = match args.tls_client_auth.as_str() {
                "disabled" => krabka_gateway::config::ClientAuthMode::Disabled,
                "optional" => krabka_gateway::config::ClientAuthMode::Optional,
                "required" => krabka_gateway::config::ClientAuthMode::Required,
...
fn load_webhooks(
    ...
    std::collections::HashMap<String, krabka_gateway::webhook_config::CompiledWebhook>,
> {
    match args.webhooks_config.as_ref() {
        Some(path) => {
            let raw = std::fs::read_to_string(path)
                .map_err(|e| anyhow::anyhow!("read webhooks config {}: {e}", path.display()))?;
            let file: krabka_gateway::webhook_config::WebhooksFile =
...
fn load_outbound(
    args: &Args,
) -> anyhow::Result<Vec<krabka_gateway::outbound_config::CompiledSubscription>> {
    match args.outbound_webhooks_config.as_ref() {
        Some(path) => {
            let raw = std::fs::read_to_string(path).map_err(|e| {
                anyhow::anyhow!("read outbound webhooks config {}: {e}", path.display())
            })?;
            let file: krabka_gateway::outbound_config::OutboundFile = toml::from_str(&raw)
...
fn build_bearer(
    args: &Args,
) -> anyhow::Result<Option<krabka_gateway::authz::auth_layer::BearerValidator>> {
...
async fn run(
    ...
    bearer: Option<krabka_gateway::authz::auth_layer::BearerValidator>,
) -> anyhow::Result<()> {
    ...
    let app = krabka_gateway::router(state.clone())
        ...
        .layer(axum::middleware::from_fn(
            krabka_gateway::authz::auth_layer::resolve_principal,
        ));
    ...
    let tls_dynamic = match config.tls.as_ref() {
        Some(t) => Some(
            krabka_gateway::serve::build_and_watch_tls(
                t.to_security(),
                t.reload_interval,
                shutdown.clone(),
            )
    ...
    krabka_gateway::serve::serve(listener, app, tls_dynamic, shutdown).await?;
    Ok(())
}
...
) -> Result<SchemaRegistryCodec, krabka_gateway::codec::CodecError> {
    let client =
...
fn build_gateway_authz(
    ...
    broker_security: Option<krabka_client_core::security::ClientSecurity>,
) -> Arc<krabka_gateway::authz::GatewayAuthz> {
    let authorizer: Arc<dyn krabka_authz::Authorizer> = match config.authz.as_ref() {
        Some(a) => Arc::new(krabka_authz::SimpleAclAuthorizer::new(
            a.super_users.iter().cloned().collect(),
        )),
        None => Arc::new(krabka_authz::AllowAllAuthorizer),
    };
    let gateway_authz = Arc::new(krabka_gateway::authz::GatewayAuthz::new(authorizer));
...
fn spawn_membership_reader(
    ...
    let group = format!("__krabka_gateway_membership_reader-{node_id}");
...
async fn spawn_outbound_subscriptions(
    ...
    let dlq_producer = Arc::new(
        krabka_client_producer::Producer::builder()
...
fn spawn_outbound_delivery(
    sub: &krabka_gateway::outbound_config::CompiledSubscription,
    config: &GatewayConfig,
    dlq_producer: Arc<krabka_client_producer::Producer>,
    shutdown: &CancellationToken,
    readiness: &Readiness,
    codec: Arc<dyn RecordCodec>,
) {
    ...
    let readiness = readiness.clone();
    tokio::spawn(async move {
        let result = krabka_gateway::outbound::run_subscription_with_policy(
            sub,
            bootstrap,
            client_id,
            dlq_producer,
...
mod tests {
    ...
    fn client_resource_policy_reads_environment_and_prefers_cli() {
        const CHILD: &str = "KRABKA_GATEWAY_CLIENT_RESOURCE_POLICY_CHILD";
        ...
        if std::env::var_os(CHILD).is_none() {
            let status =
                std::process::Command::new(std::env::current_exe().expect("test executable"))
                    ...
                    .env(CHILD, "1")
                    .env("KRABKA_GATEWAY_CLIENT_DISPATCH_QUEUE_CAPACITY", "7")
                    .env("KRABKA_GATEWAY_CLIENT_FRAME_MAX", "32KiB")
                    .status()
                    .expect("child test");
    ...
    fn runtime_cli_boundaries_precedence_and_defaults() {
        ...
        temp_env::with_vars(
            [
                ("KRABKA_GATEWAY_CONSUMER_POLL_TIMEOUT", None::<&str>),
                ("KRABKA_GATEWAY_PRODUCE_MAX_BODY", None::<&str>),
                ("KRABKA_GATEWAY_FORWARD_MAX_BODY", None::<&str>),
                ("KRABKA_GATEWAY_SCHEMA_REGISTRY_FRAME_RAW", None::<&str>),
                ("KRABKA_GATEWAY_DEDUP_WINDOW", None::<&str>),
                ("KRABKA_GATEWAY_INTERNAL_TOPIC_SEGMENT", None::<&str>),
                ("KRABKA_GATEWAY_INTERNAL_TOPIC_CREATE_TIMEOUT", None::<&str>),
            ],
            || {
                let defaults = Args::try_parse_from([
                    "krabka-gateway",
                ...
                temp_env::with_vars(
                    [
                        ("KRABKA_GATEWAY_CONSUMER_POLL_TIMEOUT", Some("701ms")),
                        ("KRABKA_GATEWAY_PRODUCE_MAX_BODY", Some("3145729B")),
                        ("KRABKA_GATEWAY_FORWARD_MAX_BODY", Some("3145728B")),
                        ("KRABKA_GATEWAY_DEDUP_WINDOW", Some("3600001ms")),
                        ("KRABKA_GATEWAY_INTERNAL_TOPIC_SEGMENT", Some("60002ms")),
                        (
                            "KRABKA_GATEWAY_INTERNAL_TOPIC_CREATE_TIMEOUT",
                            Some("10002ms"),
                        ),
                    ],
                    || {
    ...
    fn resolved_ownership_group_reaches_consumer_handoff() {
        ...
        temp_env::with_var("KRABKA_GATEWAY_DEDUP_OWNERSHIP_GROUP", None::<&str>, || {
            let defaults = Args::try_parse_from([
                "krabka-gateway",
                "--bootstrap-servers=localhost:9092",
                "--advertised-addr=localhost:9500",
