...
//! receivers dedup on X-Krabka-Event-Id.
//!
...
///
/// The task joins the consumer group `__krabka_grpc_wh_{name}` on the
...
pub async fn run_subscription(
    ...
    consumer_policy: (Option<krabka_client_core::security::ClientSecurity>, Time),
...
pub async fn run_subscription_with_policy(
    ...
    loop {
        ...
        // re-delivers (at-least-once; receivers dedup on X-Krabka-Event-Id).
...
async fn deliver_one(
    ...
    loop {
        ...
        let mut req = http
            .post(&sub.target_url)
            .header("X-Krabka-Event-Id", &event_id)
            .header("X-Krabka-Timestamp", ts.to_string())
            .body(delivery.body.clone());
        for (name, value) in &delivery.headers {
            req = req.header(name, value);
        }
        if let Some(sig) = &sig {
            req = req.header("X-Krabka-Signature", sig);
        }
...
/// `x-krabka-dlq-source` header, which holds the event id, and the
/// `x-krabka-dlq-reason` header. Otherwise log the record and drop it. The
...
async fn dead_letter(
    ...
    let prec = ProducerRecord {
        ...
        headers: vec![
            Header {
                key: "x-krabka-dlq-source".into(),
                value: Some(Bytes::from(event_id.as_bytes().to_vec())),
            },
            Header {
                key: "x-krabka-dlq-reason".into(),
...
mod tests {
    ...
    fn sub_with_decode(decode_to_json: bool) -> CompiledSubscription {
        CompiledSubscription {
            name: "dec".into(),
            group_id: "__krabka_grpc_wh_dec".into(),
    ...
    async fn structured_at_rest_is_verbatim_or_converted_for_requested_mode() {
        ...
        record.headers = vec![krabka_client_consumer::Header {
            key: "content-type".into(),
