...
async fn forward_handler(
    ...
    principal: Option<Extension<krabka_security::Principal>>,
    request: Request,
) -> Response {
...
mod tests {
    ...
    fn config(bootstrap: &str, dedup: &str, tls: Option<TlsSettings>) -> GatewayConfig {
        GatewayConfig {
            ...
            membership_topic: "__krabka_gateway_membership_fh".into(),
            tls,
    ...
    async fn forward_handler_honors_configured_body_limit_above_axum_default() {
        const DEDUP: &str = "__krabka_grpc_dedup_fh_body_limit";
    ...
    async fn forward_handler_tls_reject_uses_sentinel_coordinates() {
        const DEDUP: &str = "__krabka_grpc_dedup_fh_tls_sentinel";
        let dir = TempDir::new().unwrap();
        let broker = Broker::start(BrokerConfig::for_tests(dir.path().to_path_buf()))
            .await
            .unwrap();
    ...
    async fn forward_handler_authz_deny_uses_sentinel_coordinates() {
        const DEDUP: &str = "__krabka_grpc_dedup_fh_deny_sentinel";
        let dir = TempDir::new().unwrap();
        let broker = Broker::start(BrokerConfig::for_tests(dir.path().to_path_buf()))
            .await
            .unwrap();
    ...
    async fn forward_handler_produce_error_uses_sentinel_coordinates() {
        const DEDUP: &str = "__krabka_grpc_dedup_fh_prod_sentinel";
        let dir = TempDir::new().unwrap();
        let broker = Broker::start(BrokerConfig::for_tests(dir.path().to_path_buf()))
            .await
            .unwrap();
