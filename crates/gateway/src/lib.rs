//! `krabka-gateway` is a gRPC / Connect-RPC + HTTP gateway into Krabka topics.
//!
...
//!
//! use axum::Router;
//! use krabka_gateway::{router, state::AppState};
//!
...
///
/// `build.rs` produces the content in `OUT_DIR/krabka.gateway.v1.rs`.
///
...
pub mod pb {
    include!(concat!(env!("OUT_DIR"), "/krabka.gateway.v1.rs"));
}
