...
pub use krabka_security::ClientAuthMode;
...
impl TlsSettings {
    /// Map to the `krabka-security` config that builds server and client configs.
    #[must_use]
    pub fn to_security(&self) -> krabka_security::TlsConfig {
        krabka_security::TlsConfig {
...
impl BearerSettings {
    /// Build an [`krabka_security::OAuthBearerValidator`] from these settings.
    ///
    ...
    pub fn build(&self) -> Result<krabka_security::OAuthBearerValidator, String> {
        Ok(krabka_security::OAuthBearerValidator::Unsecured(
...
pub struct GatewayConfig {
    ...
    pub broker_security: Option<krabka_client_core::security::ClientSecurity>,
    /// Authorization settings; `None` ⇒ `AllowAll` (no enforcement; default).
...
mod tests {
    ...
    /// `krabka-security` as the raw millisecond count that its validator expects.
    #[test]
    fn bearer_clock_skew_reaches_the_validator_in_milliseconds() {
        ...
        let krabka_security::OAuthBearerValidator::Unsecured(unsecured) = validator else {
