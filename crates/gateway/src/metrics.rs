...
//!
//! Names follow the Prometheus convention `krabka_gateway_<subject>_<unit>`.
//! `Registry::with_prefix("krabka_gateway")` registers the prefix.
...
impl GatewayMetrics {
    /// Build a fresh registry with the prefix `krabka_gateway`, register all 11
    ...
    pub fn new() -> Self {
        let mut registry = Registry::with_prefix("krabka_gateway");
...
mod tests {
    ...
    fn registry_prefix_and_sends_total_present() {
        ...
        m.record_send("ok");
        let buf = encode(&m);
        // prometheus-client appends _total for Counter; prefix = krabka_gateway.
        assert2::assert!(buf.contains("krabka_gateway_sends_total"));
    }
    ...
    fn all_metrics_appear_in_encoded_output() {
        ...
        let expected = [
            "krabka_gateway_sends_total",
            "krabka_gateway_produce_latency_seconds",
            "krabka_gateway_dedup_hits_total",
            "krabka_gateway_forward_total",
            "krabka_gateway_txn_total",
            "krabka_gateway_active_subscriptions",
            "krabka_gateway_owned_partitions",
            "krabka_gateway_webhook_in_total",
            "krabka_gateway_webhook_out_total",
            "krabka_gateway_webhook_retries_total",
            "krabka_gateway_dead_letter_total",
            "krabka_gateway_request_duration_seconds_bucket",
            "krabka_gateway_request_duration_seconds_count",
            "krabka_gateway_inflight_requests",
        ];
    ...
    fn histogram_observe_appears_in_output() {
        ...
        let buf = encode(&m);
        assert2::assert!(buf.contains("krabka_gateway_produce_latency_seconds_bucket"));
        assert2::assert!(buf.contains("krabka_gateway_produce_latency_seconds_count"));
    }
