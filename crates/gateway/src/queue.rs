...
fn ack_error(error: &krabka_client_consumer::ConsumerError) -> pb::ErrorInfo {
    let code = match &error {
        krabka_client_consumer::ConsumerError::Server(code) => i32::from(*code),
        _ => 13,
    };
    pb::ErrorInfo {
        code,
        ...
        retriable: matches!(
            error,
            krabka_client_consumer::ConsumerError::Client(_)
                | krabka_client_consumer::ConsumerError::CoordinatorUnavailable
                | krabka_client_consumer::ConsumerError::CommitInvalid
                | krabka_client_consumer::ConsumerError::RebalanceFailed(_)
                | krabka_client_consumer::ConsumerError::Server(
...
async fn acknowledge_entry(
    ...
    ack: ShareAckType,
) -> Option<krabka_client_consumer::ConsumerError> {
...
pub async fn queue_acknowledge(
    ...
    for entry in req.0.entries {
        ...
        let error = match ack {
            ...
            None => Some(krabka_client_consumer::ConsumerError::IllegalState(
...
mod tests {
    ...
    fn acknowledgement_preserves_broker_error_code() {
        let error = ack_error(&krabka_client_consumer::ConsumerError::Server(121));
        assert2::assert!(
            error
                == pb::ErrorInfo {
                    code: 121,
        ...
        assert2::assert!(ack_error(&krabka_client_consumer::ConsumerError::Server(6)).retriable);
    }
