...
pub struct OutboundSubscription {
    ...
    /// `__krabka_grpc_wh_{name}`.
    pub name: String,
    /// Consumer group override. Defaults to `__krabka_grpc_wh_{name}`.
    ...
    /// `X-Krabka-Signature` header computed over the JSON envelope body.
    ...
    #[serde(
        ...
        with = "krabka_units::serde_units::human::time"
    )]
        ...
        for s in &self.subscriptions {
            ...
            let group_id = match &s.group_id {
                Some(value) => refined_type::rule::NonEmptyString::new(value.clone())
                    ...
                    .into_value(),
                None => format!("__krabka_grpc_wh_{}", s.name),
            };
...
"#;
    ...
    fn compile_valid_subscription() {
        ...
        assert2::assert!(sub.group_id.as_str() == "__krabka_grpc_wh_my-sub");
        assert2::assert!(
            sub.source_topics
                .iter()
                .map(String::as_str)
