...
use krabka_client_consumer::{AutoOffsetReset, Consumer, ConsumerRecord, IsolationLevel};
...
fn gateway_config(bootstrap: &str, client_id: &str) -> GatewayConfig {
    GatewayConfig {
        ...
        dedup_topic: "__krabka_ce_roundtrip_dedup".into(),
        dedup_partitions: 4,
        dedup_window: hours(1),
        dedup_ownership_group: "__krabka_ce_roundtrip_owners".into(),
        dedup_txn_id_prefix: format!("{client_id}-dedup"),
        advertised_addr: "127.0.0.1:0".into(),
        membership_topic: "__krabka_ce_roundtrip_membership".into(),
        tls: None,
...
fn subscription(
    ...
    CompiledSubscription {
        name: name.into(),
        group_id: format!("__krabka_ce_{name}"),
