...
KAFKA_IMAGE="${KRABKA_JVM_QUEUE_KAFKA_IMAGE:-mirror.gcr.io/apache/kafka:4.1.0}"
VALUE="${KRABKA_JVM_QUEUE_VALUE:-jvm-queue-cross-consumer}"
DRY_RUN=0
...
Runs the MSG6 JVM queue cross-consumer flow:
  ...
  3. acquire that record through the Krabka gateway QueueAcquire RPC;
...
Prerequisites for a real run: docker, curl, jq, an already-running Krabka
...
connect_url() {
  method="$1"
  printf '%s/krabka.gateway.v1.Gateway/%s' "${GATEWAY_ENDPOINT%/}" "$method"
}
