...
const N: u32 = 4;
const DEDUP_TOPIC: &str = "__krabka_wh_dedup";
const OWNERS_GROUP: &str = "__krabka_wh_dedup_owners";
...
async fn webhook_state(
    ...
    let (produce, store) = if with_dedup {
        ...
        let engine = Arc::new(DedupEngine::new(
            bootstrap,
            client_prefix,
            &format!("krabka-wh-dedup-{client_prefix}"),
            DEDUP_TOPIC.into(),
            N,
            store.clone(),
            None,
    ...
    let state = Arc::new(AppState {
        ...
        config: Arc::new(GatewayConfig {
            ...
            dedup_txn_id_prefix: format!("krabka-wh-dedup-{client_prefix}"),
            advertised_addr: "127.0.0.1:0".into(),
            membership_topic: "__krabka_wh_membership".into(),
            tls: None,
