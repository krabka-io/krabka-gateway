...
const N: u32 = 4;
const DEDUP: &str = "__krabka_grpc_dedup";
const MEMBERSHIP: &str = "__krabka_gateway_membership";
const OWNERS_GROUP: &str = "__krabka_gateway_dedup_owners";
...
/// from `krabka_metadata`'s.
fn topic_acl(
    user: &str,
    op: krabka_client_admin::AclOperation,
    topic: &str,
) -> krabka_client_admin::AclEntry {
    krabka_client_admin::AclEntry {
        resource_type: krabka_client_admin::ResourceType::Topic,
...
async fn simpleacl_allows_authorized_produce() {
    ...
    let outcomes = admin
        .create_acls(&[topic_acl(
            "alice",
            krabka_client_admin::AclOperation::Write,
            "t",
        )])
        .await
        .unwrap();
...
async fn bearer_token_resolves_principal() {
    use krabka_gateway::config::BearerSettings;
...
async fn forwarding_owner_reauthorizes_caller() {
    ...
    ensure_dedup_topic(
        ...
        N,
        hours(1),
        &krabka_gateway::dedup::topic::InternalTopicPolicy {
...
async fn count_value(bootstrap: &str, topic: &str, value: &[u8]) -> usize {
    ...
    let mut n = 0;
    for _ in 0..10 {
        let batch = consumer.poll(krabka_units::millis(500)).await.unwrap();
        for r in batch {
...
async fn spawn_acl_gateway(bootstrap: &str, client: &str) -> AclGw {
    ...
    let membership = Arc::new(MembershipStore::new());
    {
        ...
        tokio::spawn(membership.clone().run_membership(
            ...
            format!("{client}-memb"),
            MEMBERSHIP.into(),
            format!("__krabka_gateway_membership_reader-{node_id}"),
            token,
            None,
        ));
    }
    ...
    let engine = Arc::new(DedupEngine::new(
        bootstrap,
        client,
        &format!("krabka-grpc-dedup-{client}"),
        DEDUP.into(),
        N,
        store.clone(),
        None,
    ...
    let state = Arc::new(AppState {
        ...
        config: Arc::new(GatewayConfig {
            ...
            dedup_window: hours(1),
            dedup_ownership_group: OWNERS_GROUP.into(),
            dedup_txn_id_prefix: format!("krabka-grpc-dedup-{client}"),
