...
use krabka_gateway::{codec::RawCodec, produce::ProduceCore, types::GatewayRecord};
...
async fn jvm_consume_with_retry() -> Output {
    ...
    loop {
        ...
        eprintln!(
            "KRABKA[jvm_differential] attempt={attempt}/{CONSUMER_ATTEMPTS} image={IMAGE} \
    ...
    let status = loop {
        ...
        if started.elapsed() >= deadline {
            ...
            if let Err(err) = child.kill() {
                eprintln!("KRABKA[jvm_differential] kill docker run: {err}");
            }
...
fn drain(mut pipe: impl Read + Send + 'static) -> oneshot::Receiver<Vec<u8>> {
    ...
    std::thread::spawn(move || {
        ...
        if let Err(err) = pipe.read_to_end(&mut buf) {
            eprintln!("KRABKA[jvm_differential] read child pipe: {err}");
        }
