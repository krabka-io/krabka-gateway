...
//!   attempt, with `X-Krabka-Event-Id = topic-partition-offset`, a verifying
//!   `X-Krabka-Signature`, and a well-formed envelope body.
...
//!   with its value and an `x-krabka-dlq-source` header, after `max_attempts`.
...
async fn mock_handler(
    ...
    state.received.lock().unwrap().push(Received {
        body: body.to_vec(),
        event_id: get("X-Krabka-Event-Id"),
        signature: get("X-Krabka-Signature"),
        timestamp: get("X-Krabka-Timestamp"),
    });
...
fn sub(
    ...
    CompiledSubscription {
        name: name.into(),
        group_id: format!("__krabka_grpc_wh_{name}"),
...
/// `X-Krabka-Signature`. That function is `pub(crate)`, so this integration
...
/// that is, its offset, its value, and the `x-krabka-dlq-source` header. There
            ...
            for batch in batches {
                ...
                for r in &batch.records {
                    let dlq_source = r
                        .headers
                        .iter()
                        .find(|h| h.key == "x-krabka-dlq-source")
...
async fn delivers_2xx() {
    ...
    for r in &recv {
        let event_id = r.event_id.as_deref().expect("X-Krabka-Event-Id present");
        ...
        let sig = r.signature.as_deref().expect("X-Krabka-Signature present");
...
/// dead-lettered after exhaustion, with its value and an `x-krabka-dlq-source`
