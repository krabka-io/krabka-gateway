...
import { createClient, KrabkaClient } from "./client.ts";
import { KrabkaError, ErrorKind, InvalidArgumentError, NotFoundError, ServerError, UnimplementedError } from "./errors.ts";
...
class Adapter {
  private client: KrabkaClient = createClient({ endpoint: "mock://gateway" });
...
async function safeHandle(adapter: Adapter, command: Command): Promise<unknown> {
  try {
    ...
    return sdkError(toKrabkaError(error));
  }
}
...
function sdkError(error: KrabkaError): unknown {
...
function toKrabkaError(error: unknown): KrabkaError {
