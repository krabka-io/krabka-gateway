...
Reference Go slice for the serverless SDK contract. It includes generated Connect clients for `krabka.gateway.v1`, a small `krabka` facade, mock/unreachable endpoints for conformance, and the JSON-lines conformance adapter for contract v1.1.
...
- `Messaging().PublishEvent(ctx, topic, CloudEvent{...})` maps CloudEvents to the Krabka binary-mode headers: `ce_id`, `ce_source`, `ce_type`, `ce_specversion`, and bare `content-type` for `datacontenttype`.
