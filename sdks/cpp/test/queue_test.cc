...
void expect_error(const krabka::ErrorKind kind, const std::string& message,
...
int main() {
  ...
  (void)client.publish(krabka::Record{
      .topic = "jobs",
      .value = bytes("one"),
      .headers = {{"kind", bytes("queue")}, {"nullable", std::nullopt}},
  });
  (void)client.publish(
      krabka::Record{.topic = "jobs", .value = bytes("two"), .headers = {}});
  (void)client.publish(
      krabka::Record{.topic = "other", .value = bytes("skip"), .headers = {}});
  ...
  assert(renewed.results[1].error->kind == krabka::ErrorKind::InvalidArgument);
  assert(renewed.results[1].error->message == "queue message is not acquired");

  const auto released = client.queue_acknowledge(
      first.session_id,
      {{.message_id = "jobs:0:0", .ack_type = krabka::QueueAckType::Release}});
  ...
  const auto accepted = client.queue_acknowledge(
      ...
      {{.message_id = "jobs:0:0", .ack_type = krabka::QueueAckType::Accept},
       {.message_id = "missing:0:0", .ack_type = krabka::QueueAckType::Reject}});
  ...
  expect_error(krabka::ErrorKind::InvalidArgument, "queue group is required", [&] {
  ...
  expect_error(krabka::ErrorKind::InvalidArgument, "queue session_id is required", [&] {
  ...
  krabka::Client isolated;
  (void)isolated.publish(
      krabka::Record{.topic = "jobs", .value = bytes("one"), .headers = {}});
  ...
  const auto wrong_ack = isolated.queue_acknowledge(
      other.session_id,
      {{.message_id = owner.messages[0].message_id, .ack_type = krabka::QueueAckType::Accept}});
  ...
  expect_error(krabka::ErrorKind::InvalidArgument, "queue session expired; re-acquire", [&] {
  ...
  krabka::Client independent_groups;
  ...
  const auto group_b_accepted = independent_groups.queue_acknowledge(
      ...
      {{.message_id = group_b.messages[0].message_id,
        .ack_type = krabka::QueueAckType::Accept}});
  ...
  expect_error(krabka::ErrorKind::Transport, "endpoint unreachable", [&] {
