...
std::optional<std::string> header_value(const krabka::LiveTransportRequestForTest& request,
...
int main() {
  const krabka::Record record{.topic = "t", .value = bytes("v"), .headers = {{"h", bytes("x")}}};
  const auto send_request = krabka::live_transport_send_request_bytes_for_test(record);
  assert(send_request == std::vector<std::uint8_t>({0x0a, 0x0e, 0x0a, 0x01, 't', 0x1a, 0x01, 'v',
                                                    0x22, 0x06, 0x0a, 0x01, 'h', 0x12, 0x01, 'x'}));
  const auto send_http = krabka::live_transport_send_http_request_for_test(record, "token");
  assert(send_http.path == "/krabka.gateway.v1.Gateway/Send");
  ...
  const auto send_body_plan = krabka::live_transport_request_body_read_plan_for_test(send_http);
  ...
  const auto subscribe_start = krabka::live_transport_subscribe_start_bytes_for_test({"t"}, "g", std::nullopt);
  assert(subscribe_start == std::vector<std::uint8_t>({0x0a, 0x08, 0x0a, 0x01, 'g', 0x12, 0x01, 't', 0x18, 0x01}));
  const auto subscribe_http = krabka::live_transport_subscribe_http_request_for_test({"t"}, "g", std::nullopt, std::nullopt);
  assert(subscribe_http.path == "/krabka.gateway.v1.Gateway/Subscribe");
  ...
  const auto escaped_filter_start = krabka::live_transport_subscribe_start_bytes_for_test(
      {"t"}, "g", krabka::Filter{.path = "$.path", .op = "equals",
                                  .value = krabka::json::Value{std::string{"C:\\tmp\\O'Brien"}}});
  ...
  const auto subscribe_body_plan = krabka::live_transport_request_body_read_plan_for_test(subscribe_http);
  ...
  const auto queue_acquire_http = krabka::live_transport_queue_acquire_http_request_for_test(
      "t", "g", 1, 30'000, "actual", "token");
  assert(queue_acquire_http.path == "/krabka.gateway.v1.Gateway/QueueAcquire");
  ...
  const std::vector<krabka::QueueAckEntry> ack_entries{
      {.message_id = "t:0:0", .ack_type = krabka::QueueAckType::Accept}};
  const auto queue_ack_http = krabka::live_transport_queue_acknowledge_http_request_for_test(
      "actual", ack_entries, std::nullopt);
  assert(queue_ack_http.path == "/krabka.gateway.v1.Gateway/QueueAcknowledge");
  assert(queue_ack_http.body == std::vector<std::uint8_t>(
                                    {0x0a, 0x06, 'a', 'c', 't', 'u', 'a', 'l',
                                     0x12, 0x05, 0x0a, 0x01, 't', 0x20, 0x01}));

  const std::vector<krabka::QueueRenewEntry> renew_entries{{.message_id = "t:0:0"}};
  const auto queue_renew_http = krabka::live_transport_queue_renew_http_request_for_test(
      "actual", renew_entries, std::nullopt);
  assert(queue_renew_http.path == "/krabka.gateway.v1.Gateway/QueueRenew");
  assert(queue_renew_http.body == queue_ack_http.body);

  try {
    (void)krabka::live_transport_queue_acknowledge_http_request_for_test(
        "actual", {{.message_id = "invalid", .ack_type = krabka::QueueAckType::Accept}},
        std::nullopt);
    assert(false);
  } catch (const krabka::SdkError& error) {
    assert(error.error().kind == krabka::ErrorKind::InvalidArgument);
    assert(error.error().message ==
           "queue message_id must be <topic>:<partition>:<offset>");
  }

  const auto queue_acquire_response = krabka::live_transport_queue_acquire_response_for_test(
  ...
  const auto queue_payload_presence = krabka::live_transport_queue_acquire_response_for_test(
  ...
  const std::vector<krabka::QueueAckEntry> mixed_entries{
      {.message_id = "t:0:0", .ack_type = krabka::QueueAckType::Accept},
      {.message_id = "missing:0:0", .ack_type = krabka::QueueAckType::Accept}};
  const auto queue_ack_response = krabka::live_transport_queue_acknowledge_response_for_test(
  ...
  assert(queue_ack_response.results[1].error->kind == krabka::ErrorKind::InvalidArgument);
  assert(queue_ack_response.results[1].error->message == "bad");

  const std::vector<krabka::QueueAckEntry> one_entry{
      {.message_id = "t:0:0", .ack_type = krabka::QueueAckType::Accept}};
  const auto not_acquired = krabka::live_transport_queue_acknowledge_response_for_test(
      queue_error_response(9, "record is not acquired by this session"), one_entry);
  assert(not_acquired.results[0].error->kind == krabka::ErrorKind::InvalidArgument);
  assert(not_acquired.results[0].error->message == "queue message is not acquired");
  const auto retriable_invalid = krabka::live_transport_queue_acknowledge_response_for_test(
      queue_error_response(9, "coordinator retry", true), one_entry);
  assert(retriable_invalid.results[0].error->kind == krabka::ErrorKind::Transport);
  assert(retriable_invalid.results[0].error->message == "coordinator retry");

  const auto queue_renew_response = krabka::live_transport_queue_renew_response_for_test(
  ...
  const auto stream_close_plan = krabka::live_transport_stream_close_plan_for_test();
  ...
  const auto send_safety = krabka::live_transport_send_safety_for_test();
  assert(send_safety.suppresses_sigpipe);
#ifdef MSG_NOSIGNAL
  assert(send_safety.uses_msg_nosignal);
#endif

  assert(!krabka::detail::http2_parse_failure_message(0, "parse error").has_value());
  assert(!krabka::detail::http2_parse_failure_message(42, "parse error").has_value());
  const auto parse_failure = krabka::detail::http2_parse_failure_message(-902, "parse error");
  assert(parse_failure.has_value());
  assert(parse_failure->find("parse HTTP/2 data failed") == 0);

  const auto response_lifecycle = krabka::live_transport_response_lifecycle_for_test();
  ...
  assert(response_lifecycle.connect_http_error->kind == krabka::ErrorKind::InvalidArgument);
  ...
  assert(response_lifecycle.connect_end_stream_error->kind == krabka::ErrorKind::NotFound);
  assert(response_lifecycle.connect_end_stream_error->message == "missing");

  krabka::Client client;
  const auto published = client.publish(krabka::Record{.topic = "t", .value = bytes("v"), .headers = {}});
  assert(published.offset == 0);
  auto stream = client.subscribe({"t"}, "g", std::nullopt);
  assert(stream.next(0).value == bytes("v"));
  stream.close();

  krabka::Client live("http://127.0.0.1:1");
  try {
    (void)live.subscribe(
        {"t"}, "g",
        krabka::Filter{.path = "kind", .op = "equals",
                       .value = krabka::json::Value{std::string{"keep"}}});
    assert(false);
  } catch (const krabka::SdkError& error) {
    assert(error.error().kind == krabka::ErrorKind::InvalidArgument);
  ...
  krabka::Client unreachable("unreachable://gateway");
  try {
    (void)unreachable.publish(krabka::Record{.topic = "t", .value = bytes("v"), .headers = {}});
    assert(false);
  } catch (const krabka::SdkError& error) {
    assert(error.error().kind == krabka::ErrorKind::Transport);
  }
  try {
